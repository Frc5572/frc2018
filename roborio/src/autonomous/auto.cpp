...
AUTO_FUNC(P2toSwR7,.45,  (), (intake_f(OUTTAKE_V,0.8);),
		0, 60);

AUTO_FUNC(P2toSwR6, .45, (), (P2toSwR7(ALL_THE_VARS);),
		-10, -20);

AUTO_FUNC(P2toSwR5, .45, (std::thread t([&]() { frc::Timer timer; SmartDashboard::PutNumber("Auto Seq",0);timer.Start(); while(timer.Get() < 1.5) { lift_f(true, LIFT_V); } lift_f(true, 0.0); SmartDashboard::PutNumber("Auto Seq",1); });), (t.join();P2toSwR6(ALL_THE_VARS);),
		0, -20);
...
AUTO_FUNC(P3toSwR2,.45,  (), (intake_f(OUTTAKE_V,0.8);),
		-15, 15)
